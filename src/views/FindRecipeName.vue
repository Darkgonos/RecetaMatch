...
export default {
  ...
  watch: {
    '$route': 'loadRecipesNameOnRouteChange'
  },
  methods: {
    addIngredient() {
    ...
    async loadRecipesNameOnRouteChange() {
      try {
        const name = this.$route.params.recipeName;
        await this.loadRecipesName(name);
  ...
  mounted() {
    this.loadRecipesNameOnRouteChange();
  }
};
</script>
